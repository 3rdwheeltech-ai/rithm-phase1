"""conversation baseline — conversation.sessions, conversation.messages

Transcribed from ops/db/04_conversation.sql (as-built local schema).
Assumes the 'conversation' schema and public.touch_updated_at() already exist.

Revision ID: 0001_conversation_baseline
Revises:
...
revision: str = "0001_conversation_baseline"
...
def upgrade() -> None:
    op.execute(
        ...
        DROP TRIGGER IF EXISTS sessions_touch ON conversation.sessions;
